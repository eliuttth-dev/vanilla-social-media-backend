...
export default async function registerMiddleware(req: Request, res: Response, next: NextFunction): Promise<void> {
  ...
  if (!emailRegex.test(email)) {
    res.status(400).json({
      status: "error",
      message: "Email format is not correct. Please try again.",
    });
    return;
  }
  ...
  if (!passwordRegex.test(password)) {
    res.status(400).json({
      status: "error",
      message: "Password should contain: 1 UpperCase letter, 1 LowerCase letter, 1 number and at least 6 characters long.",
    });
    return;
  }
